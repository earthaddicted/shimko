...
(function() {
	if ($('.blog-block__item').length) {
		...
		.fromTo('.blog-block__img img', 4, { y:0}, { y:-100})
...
(function() {
	...
	if ($('.slider-multiple').length) {
		...
		function activeSlick() {
			...
			$('.slider-multiple').on('init', function(slick) {
                $('.slider-multiple').fadeTo("slow", 1);
            });
...
//item0.button0.onCLick = f{item0.toggle}
//item1.buttton1.onCLick = f{item1.toggle}

//[0 : item0, 1 : item1, 2 : item2, 3 : item3]

const iterator = (index, item) => {
 	const button = $(item).find(".arrow-action");
 	const clickHandler = () => {
